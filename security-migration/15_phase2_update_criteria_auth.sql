-- ============================================================================
-- المرحلة 2 (أمان) — حفظ معايير التقييم عبر RPC مُصادَق + تحقّق بنيوي
-- ============================================================================
-- admin_update_criteria: admin فقط، رمز إلزامي، يتحقّق من بنية المعايير قبل
--   الكتابة (يحمي منطق الاحتساب)، ويحدّث criteria_config (config_key='criteria').
--   الاستبدال يغطّي "استعادة الافتراضي" (الواجهة ترسل DEFAULT_CRITERIA).
-- ============================================================================
...
begin
  ...
  -- تحقّق بنيوي (coalesce على jsonb_typeof لأن المفتاح الغائب يُرجع NULL)
  ...
  for v_section in select value from jsonb_array_elements(p_criteria->'sections') loop
    ...
    for v_sub in select value from jsonb_array_elements(v_section->'subsections') loop
      -- الأقسام الفرعية تملك weight/items فقط (لا key)
