-- =========================================================================
-- 48_create_evaluation_template_id.sql  (Phase 2b — اختيار القالب عبر template_id)
--
-- create_evaluation يحلّ القالب عبر users.template_id أولاً (يشتقّ v_type/v_job/v_template
-- منه)، ثم fallback للمنطق الحالي (dept, job_role) في كلا الفرعين (CG + mahzam).
--
-- CREATE OR REPLACE (لا DROP): التوقيع لا يتغيّر (template_id من users، ليس معاملاً).
-- المتن حرفي من ملف 43 (matches_repo=true مؤكّد: live == 43، الجسم متطابق 100%) عدا:
--   • v_tid في declare + قراءته من users
--   • كتلة حلّ template_id قبل كشف v_type (block 1)
--   • حراسة اختيار الفرعين بـ if v_template is null (block 2 CG + block 3 mahzam)
-- يحفظ حرفياً: ★#33 (تطبيع week_start) + ★ملف 30 (pass_score) + ★م23 (مطابقة الدور).
-- الأخطاء المُنظَّمة «لا نموذج» تبقى (create_evaluation يمنع التقييمات الفارغة أصلاً؛ القيادات
-- محجوبون مبكراً بـ«الموظف غير مرتبط بقسم») — لا RAISE (بخلاف 49). المرجع: #38. التاريخ: 2026-08-02.
-- =========================================================================
BEGIN;
...
-- المنح (نمط ملف 43؛ CREATE OR REPLACE يحفظها أصلاً)
...
DO $$
BEGIN
  IF has_function_privilege('public', 'public.create_evaluation(text,bigint,date,text,text,text,text,text,jsonb,text,text,numeric,text,text,text,date,date,jsonb)', 'EXECUTE') THEN
    RAISE EXCEPTION 'GRANT check failed: create_evaluation يجب ألّا تكون قابلة للتنفيذ من PUBLIC';
  END IF;
  RAISE NOTICE '✅ SQL 48: create_evaluation public EXECUTE = false';
END $$;
