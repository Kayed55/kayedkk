-- ============================================================================
-- Migration 55 (PR #70) — create_evaluation: مهلة اعتراض 24h + qo_suggestion (CG path فقط)
-- الأساس: الجسم الحرفي من الإنتاج (CSV 2026-08-06) + تعديل واحد في فرع pdf_based_weekly فقط:
--   ① insert/on-conflict في creative_gene_weekly_status: أضف objection_deadline = now()+interval '24 hours' (D3)
-- (توصية الجودة D1 تبقى عبر cg_set_eval_action الموجود — لا نكرّرها هنا.)
-- ★ مسار محزم (section_based) — بايت-ببايت كما هو (صفر تغيير). grants محفوظة (CREATE OR REPLACE).
-- ============================================================================
...
begin
  ...
  if v_type = 'pdf_based_weekly' then
    ...
    values(v_id, p_employee_id, v_sess.user_id, v_we, v_items, v_scores->'section_scores',
      ...
      v_pass,                                                                           -- ★ ملف 30
      now(), now());
    -- ★ #70 (D3): objection_deadline = now()+24h (مهلة الاعتراض تبدأ من حفظ الجودة)
    insert into public.creative_gene_weekly_status(employee_id, week_start, week_end, status, workflow_state, evaluation_id, pdf_file_path, pdf_uploaded_at, pdf_uploaded_by, objection_deadline, updated_at)
    values(p_employee_id, v_ws, v_we, 'evaluated', 'pending_supervisor', v_id, trim(p_pdf_file_path), coalesce(v_up_at, now()), coalesce(v_up_by, p_employee_id::int), now()+interval '24 hours', now())
    on conflict (employee_id, week_start) do update set status='evaluated', workflow_state='pending_supervisor', evaluation_id=v_id, objection_deadline=now()+interval '24 hours', updated_at=now()
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname='create_evaluation' AND pronamespace='public'::regnamespace) THEN
    RAISE EXCEPTION 'Migration 55 failed — create_evaluation missing';
  END IF;
  RAISE NOTICE 'Migration 55 OK — CG path: objection_deadline=24h + qo_suggestion; section_based unchanged';
END $$;

COMMIT;

-- Rollback: أعد الجسم الحرفي الأصلي (بلا objection_deadline في creative_gene_weekly_status، بلا qo_suggestion insert).
