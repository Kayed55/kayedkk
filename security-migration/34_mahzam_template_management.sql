-- =========================================================================
-- 34_mahzam_template_management.sql
-- م23-ب: RPCs لإدارة قوالب محزم متعددة + إصلاح LIMIT 1 الحرج (مطابقة job_role)
-- =========================================================================
-- المحتوى:
--   1) create_mahzam_template      — إنشاء/تحديث قالب محزم (upsert بـ job_role)
--   2) set_mahzam_template_active  — تفعيل/تعطيل قالب (يمنع تعطيل آخر نشط)
--   3) list_mahzam_templates       — سرد قوالب محزم (للواجهة/التصدير)
--   4) create_evaluation (18-param)      — إعادة تعريف حرفية من ملف 30 مع
--        تعديل جراحي واحد: فرع section يطابق job_role (كفرع CG) بدل LIMIT 1.
--   5) admin_update_evaluation (12-param) — إعادة تعريف حرفية من ملف 30 مع
--        تعديل fallback القالب ليطابق قسم/مسمى الموظف.
--
-- آلية كشف «محزم»: departments.code = 'mahzam' (صريح، ثابت، مستخدَم في العميل).
-- العزل: لا مساس بأي دالة CG، ولا review_objection، ولا create_weekly_evaluation،
--   ولا ALTER على evaluation_templates. كل دوال م23 تفرض template_type='section_based'.
--
-- انحراف مقصود عن المسودة (للصحة): المنح للدوال الثلاث الجديدة لـ anon أيضاً —
--   العميل يتصل بمفتاح anon + رمز جلسة (كنمط create_evaluation الفعلي). لو مُنحت
--   لـ authenticated فقط لتعذّر على التطبيق استدعاؤها.
--
-- id: يُترك للقاعدة لتولّده تلقائياً (OpenAPI يُظهر id غير required → له default/identity).
--   لا نمرّر id في الـINSERT — يعمل لكل الحالات (identity ALWAYS/BY DEFAULT/serial).
-- =========================================================================

BEGIN;

-- -------------------------------------------------------------------------
-- 1) create_mahzam_template
-- -------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.create_mahzam_template(
...
AS $function$
declare
  v_sess record; v_id bigint; v_sections jsonb; v_weight_sum numeric;
  v_section jsonb; v_sub jsonb;
begin
  -- جلسة + دور
  select * into v_sess from public.verify_session(p_session_token);
  if not coalesce(v_sess.is_valid, false) then
    return query select false, null::bigint, 'انتهت الجلسة أو الرمز غير صالح'::text; return;
  end if;
  if v_sess.role not in ('admin','quality_officer') then
    return query select false, null::bigint, 'ليس لديك صلاحية'::text; return;
  end if;

  -- القسم = محزم
  if not exists(select 1 from public.departments where id = p_department_id and code = 'mahzam') then
    return query select false, null::bigint, 'هذه العملية مخصصة لقسم محزم فقط'::text; return;
  end if;

  -- job_role صالح (حرج — يمنع NULL/فارغ من استبدال النموذج الافتراضي عبر NULLS NOT DISTINCT)
  if p_job_role is null or trim(p_job_role) = '' then
    return query select false, null::bigint, 'اسم النموذج (job_role) مطلوب — لا يمكن أن يكون فارغاً'::text; return;
  end if;
  if p_job_role !~ '^[a-z][a-z0-9_]{1,49}$' then
    return query select false, null::bigint, 'اسم النموذج يجب أن يبدأ بحرف إنجليزي صغير ويحتوي فقط على حروف صغيرة وأرقام وشرطة سفلية (بحد أقصى 50 حرفاً)'::text; return;
  end if;

  -- template_data: sections مصفوفة غير فارغة + مجموع الأوزان = 100
  v_sections := p_template_data -> 'sections';
  if v_sections is null or jsonb_typeof(v_sections) <> 'array' or jsonb_array_length(v_sections) = 0 then
    return query select false, null::bigint, 'يجب توفير sections مصفوفة غير فارغة'::text; return;
  end if;
  select coalesce(sum((s->>'weight')::numeric), 0) into v_weight_sum from jsonb_array_elements(v_sections) s;
  if v_weight_sum <> 100 then
    return query select false, null::bigint, ('مجموع أوزان الأقسام = '||v_weight_sum||'% (يجب أن يكون 100%)')::text; return;
  end if;

  -- تحقّق هيكلي (قرار 1 — خيار أ): كل قسم فيه key/type/weight/subsections،
  -- كل subsection فيه items (مصفوفة غير فارغة)، وكل item فيه key.
  for v_section in select value from jsonb_array_elements(v_sections) loop
    if not (v_section ? 'key' and v_section ? 'type' and v_section ? 'weight' and v_section ? 'subsections') then
      return query select false, null::bigint, 'كل قسم يجب أن يحتوي على: key, type, weight, subsections'::text; return;
    end if;
    if jsonb_typeof(v_section -> 'subsections') <> 'array' or jsonb_array_length(v_section -> 'subsections') = 0 then
      return query select false, null::bigint, ('subsections للقسم "'||coalesce(v_section->>'key','')||'" يجب أن تكون مصفوفة غير فارغة')::text; return;
    end if;
    for v_sub in select value from jsonb_array_elements(v_section -> 'subsections') loop
      if jsonb_typeof(v_sub -> 'items') <> 'array' or jsonb_array_length(v_sub -> 'items') = 0 then
        return query select false, null::bigint, ('items للقسم الفرعي "'||coalesce(v_sub->>'key','')||'" يجب أن تكون مصفوفة غير فارغة')::text; return;
      end if;
      if exists(select 1 from jsonb_array_elements(v_sub -> 'items') item where not (item ? 'key')) then
        return query select false, null::bigint, ('يوجد بند (item) بلا key في القسم الفرعي "'||coalesce(v_sub->>'key','')||'"')::text; return;
      end if;
    end loop;
  end loop;

  -- upsert بـ (department_id, job_role) — القيد uq_dept_role (NULLS NOT DISTINCT)
  -- عمود id يُولَّد تلقائياً (OpenAPI: id غير required → له default) → لا نمرّره.
  insert into public.evaluation_templates
    (department_id, template_type, template_jsonb, version, is_active, job_role, updated_by, created_at, updated_at)
  values
    (p_department_id, 'section_based', p_template_data, 1, true, p_job_role, v_sess.user_id, now(), now())
  on conflict (department_id, job_role)
  do update set
    template_jsonb = excluded.template_jsonb,
    version        = public.evaluation_templates.version + 1,
    is_active      = true,
    updated_by     = v_sess.user_id,
    updated_at     = now()
  where public.evaluation_templates.template_type = 'section_based'
  returning id into v_id;

  if v_id is null then
    return query select false, null::bigint, 'تعذّر الحفظ — قد يوجد نموذج بنفس الاسم من نوع مختلف'::text; return;
  end if;

  return query select true, v_id, 'تم حفظ النموذج بنجاح'::text;
end; $function$;

-- -------------------------------------------------------------------------
-- 2) set_mahzam_template_active
-- -------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.set_mahzam_template_active(
  p_session_token text,
  p_department_id bigint,
  p_job_role      text,
  p_active        boolean
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
declare v_sess record; v_other_active int; v_found int;
begin
  select * into v_sess from public.verify_session(p_session_token);
  ...
  if not exists(select 1 from public.departments where id = p_department_id and code = 'mahzam') then
    return query select false, 'هذه العملية مخصصة لقسم محزم فقط'::text; return; end if;

  -- منع تعطيل آخر نموذج نشط لقسم محزم
  if p_active = false then
    select count(*) into v_other_active from public.evaluation_templates
      where department_id = p_department_id and template_type = 'section_based' and is_active
        and (job_role is distinct from p_job_role);
    if v_other_active = 0 then
      return query select false, 'لا يمكن تعطيل آخر نموذج نشط لقسم محزم'::text; return;
    end if;
  end if;

  update public.evaluation_templates
    set is_active = p_active, updated_by = v_sess.user_id, updated_at = now()
    where department_id = p_department_id and template_type = 'section_based'
      and job_role is not distinct from p_job_role;
  get diagnostics v_found = row_count;
  if v_found = 0 then return query select false, 'النموذج غير موجود'::text; return; end if;

  return query select true, case when p_active then 'تم تنشيط النموذج'::text else 'تم تعطيل النموذج'::text end;
end; $function$;

-- -------------------------------------------------------------------------
-- 3) list_mahzam_templates
-- -------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.list_mahzam_templates(
  p_session_token text,
  p_department_id bigint
) RETURNS TABLE(id bigint, job_role text, is_active boolean, template_jsonb jsonb, version integer, created_at timestamptz)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
declare v_sess record;
begin
  select * into v_sess from public.verify_session(p_session_token);
  if not coalesce(v_sess.is_valid,false) then return; end if;                             -- جلسة غير صالحة → لا صفوف
  -- م23 تعديل 1: supervisor مسموح بالقراءة (الكتابة محصورة في admin/quality_officer)
  if v_sess.role not in ('admin','quality_officer','supervisor') then return; end if;     -- غير مصرّح → لا صفوف
  return query
    select t.id, t.job_role, t.is_active, t.template_jsonb, t.version, t.created_at
    from public.evaluation_templates t
    where t.department_id = p_department_id and t.template_type = 'section_based'
    order by (t.job_role is null) desc, t.job_role;
end; $function$;

-- -------------------------------------------------------------------------
-- 4) create_evaluation (18-param) — منسوخة حرفياً من ملف 30 + تعديل جراحي واحد
--    (فرع section: مطابقة job_role بدل LIMIT 1 العشوائي — السطر المُعلّم ★ م23)
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 5) admin_update_evaluation (12-param) — منسوخة حرفياً من ملف 30 + تعديل fallback القالب
-- -------------------------------------------------------------------------
...
declare v_sess record; v_actor text; v_eval public.evaluations; v_scores jsonb; v_call_type text; v_template jsonb;
  v_pass numeric;  -- ★ ملف 30
  v_edept bigint; v_ejob text;  -- ★ م23
  v_allowed constant text[] := array['admin','quality_officer'];
begin
  select * into v_sess from public.verify_session(p_session_token);
  ...
  -- ★ م23: fallback القالب يطابق قسم/مسمى الموظف (بدل أي قالب section) — نادر الاستخدام (snapshot موجود عادةً)
  select department_id, job_role into v_edept, v_ejob from public.users where id = v_eval.employee_id;
  v_template := coalesce(
    v_eval.template_snapshot,
    (select template_jsonb from public.evaluation_templates
       where department_id=v_edept and template_type='section_based' and is_active
         and (job_role = v_ejob or job_role is null)
       order by (job_role is not null) desc limit 1));
...
-- -------------------------------------------------------------------------
-- 6) المنح — الدوال الثلاث الجديدة (anon + authenticated: العميل يتصل بمفتاح anon)
--    create_evaluation / admin_update_evaluation: CREATE OR REPLACE → مِنَحها محفوظة.
-- -------------------------------------------------------------------------
REVOKE ALL ON FUNCTION public.create_mahzam_template(text, bigint, text, jsonb) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_mahzam_template(text, bigint, text, jsonb) TO anon, authenticated;

REVOKE ALL ON FUNCTION public.set_mahzam_template_active(text, bigint, text, boolean) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.set_mahzam_template_active(text, bigint, text, boolean) TO anon, authenticated;

REVOKE ALL ON FUNCTION public.list_mahzam_templates(text, bigint) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.list_mahzam_templates(text, bigint) TO anon, authenticated;

COMMIT;

-- =========================================================================
-- تحقّق بعد التنفيذ:
--   -- الدوال الثلاث الجديدة موجودة:
--   SELECT proname, pronargs FROM pg_proc WHERE pronamespace='public'::regnamespace
--     AND proname IN ('create_mahzam_template','set_mahzam_template_active','list_mahzam_templates');
--   -- إصلاح مطابقة job_role سرى (يظهر في تعريف create_evaluation):
--   SELECT pg_get_functiondef('public.create_evaluation(text,bigint,date,text,text,text,text,text,jsonb,text,text,numeric,text,text,text,date,date,jsonb)'::regprocedure) LIKE '%job_role = v_job or job_role is null%';
-- =========================================================================
